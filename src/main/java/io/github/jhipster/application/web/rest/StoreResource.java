...
/**
 * REST controller for managing {@link io.github.jhipster.application.domain.Store}.
 */
@RestController
@RequestMapping("/api")
public class StoreResource {
     ...
     */
    @GetMapping("/stores")
    public List<Store> getAllStores(@RequestParam(required = false) String filter) {
        if ("country-is-null".equals(filter)) {
            log.debug("REST request to get all Stores where country is null");
            return StreamSupport
                .stream(storeRepository.findAll().spliterator(), false)
                .filter(store -> store.getCountry() == null)
